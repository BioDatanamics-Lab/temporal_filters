...
elseif KSE == 2
    ...
    [deltasmax,fdeltasres] = max(DeltaSfp);
...
%     plot(SpkFreqin,Zfp,'-g','linewidth',2);
%     plot(SpkFreqin,DeltaSfp,'-b','linewidth',2);
    plot(SpkFreqin,F,'--r','linewidth',2);
    plot(SpkFreqin,G,'--g','linewidth',2);
    plot(SpkFreqin,F.*G,'--b','linewidth',2);
    axis([0 SpkFreqinmax 0  1]);
    h = legend('$\bar{F}$','$\bar{G}$','$\bar{F} \bar{G}$');
    set(h,'interpreter','Latex')        
    set(gca,'fontsize',24);
    xlabel('f_{spk}  [Hz]');
    ylabel('');
